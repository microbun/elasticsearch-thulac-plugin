 ...
 * separated parts their respective tags. A {@link org.thunlp.thulac.data.Dat} file stores the list of negative
 * phrases to be separated by {@link #process(java.util.List)}.
 */
	...
	public void process(List<TaggedWord> sentence) {
		...
		for (int i = sentence.size() - 1; i >= 0; --i) {
			...
			if (this.negPhrases.contains(tagged.word)) {
				...
				sentence.add(i + 1, new TaggedWord(word, "v",tagged.startOffset+1,tagged.endOffset));
				sentence.get(i).endOffset = sentence.get(i).startOffset +1;
