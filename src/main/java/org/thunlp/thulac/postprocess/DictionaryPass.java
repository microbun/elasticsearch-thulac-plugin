 ...
 * The {@link #process(java.util.List)} method tends to find the longest concatenation of words
 ...
 * to the word list, which makes the final output from {@link #process(java.util.List)} {@code
 * "ABC", "DE"}.<br>
 * Please notice that although {@code "ABCD"} exists in {@link #dictionary}, the
 * {@link #process(java.util.List)} method will not attempt to split whole words apart.
 */
public class DictionaryPass implements IPostprocessPass {
	...
	public void process(List<TaggedWord> sentence) {
		...
		for (int i = 0, size = sentence.size(); i < size; i++) {
			...
			Integer startOffset = sentence.get(i).startOffset;
			sentence.set(i, new TaggedWord(longest,tag, startOffset,startOffset+longest.length()));
